...
## Summary Option 4 verification — 2026-09-23

The Summary production Compose fixtures were rerun to
`desktop/build/reports/summary-option-4/` for the selected
[Option 4 reference](../.mockups/screens/summary-redesign/option-4.html). These are
offscreen captures only. No native-window or screen-reader evidence is claimed,
and no density combinations beyond those actually captured are claimed.

The first full desktop gate failed at `spotlessKotlinCheck` because
`ProjectSummaryPane.kt` needed formatting. After formatting, the desktop gate and
focused visual/accessibility/keyboard rerun passed. The horizontal overview
assertion is included in `DesktopVisualLayoutTest`; the populated-wide overview
render was generated by that focused rerun. Review compared the populated wide
composition with optional details collapsed and used the existing rendered
fixtures for no-project, unavailable/zero, empty-selection, lifecycle, stale,
failed/partial, long-content, and delayed-detail states. A later offscreen rerun
confirmed the fresh overview dot matches the green Updated status; the rendered
status test also samples the stale and failed dots. That run's first desktop gate
failed with a `ConcurrentModificationException` in
`DesktopWorkflowPresenterTest.verifiedScanTrustsBeforeStartingAndNavigationSendsNoTrustRequest`;
a standalone rerun of the same gate passed. Native-window checks remain unavailable.
