...
func TestCachedFileAnalysisMarksV7PromptResultsStale(t *testing.T) {
	service, _ := newSemanticAnalysisService(t, "http://127.0.0.1:1", 0)
	prepared, err := service.prepareFileAnalysis("main.go")
	if err != nil {
		t.Fatal(err)
	}
	if got := EngineeringInsightPromptVersion(); got != "file-analysis-v8" {
		t.Fatalf("file analysis prompt version = %q, want file-analysis-v8", got)
	}
	legacy := project.FileAnalysis{
		...
		ReasoningEffort: prepared.input.ReasoningEffort, PromptVersion: "file-analysis-v7", ContextPolicyVersion: prepared.input.ContextPolicyVersion,
	}
	...
	if err != nil || cached.Status != project.AnalysisStatusStale {
		t.Fatalf("v7 cache = %+v, %v; want stale", cached, err)
	}
}
