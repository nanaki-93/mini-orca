...
// ResolvePathForWrite validates a relative output path whose file may not exist yet.
func ResolvePathForWrite(root, relative string) (string, error) {
	if strings.TrimSpace(relative) == "" {
		return "", fmt.Errorf("file path is required")
	}
	if filepath.IsAbs(relative) {
	...
	candidate := filepath.Join(root, filepath.Clean(relative))
	lexicalRel, err := filepath.Rel(root, candidate)
...
// ResolveDirectory safely resolves a relative directory inside root.
