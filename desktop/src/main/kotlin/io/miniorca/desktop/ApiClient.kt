...
@Serializable private data class ProjectImportRequest(
    @SerialName("project_path") val projectPath: String,
    @SerialName("confirm_remote_provider") val confirmRemoteProvider: Boolean,
)

@Serializable private data class ProjectRestoreRequest(@SerialName("project_path") val projectPath: String)
@Serializable private data class ProjectRevisionRequest(@SerialName("project_revision") val projectRevision: String)

@Serializable private data class FileAnalysisRequest(
    val path: String,
...
@Serializable private data class FindingStatusRequest(
    @SerialName("project_revision") val projectRevision: String,
    val status: String,
)

@Serializable private data class AnalyzeAllStartRequest(
    @SerialName("project_revision") val projectRevision: String,
...
@Serializable private data class AnalyzeAllResumeRequest(
    @SerialName("project_revision") val projectRevision: String,
    @SerialName("confirm_remote_provider") val confirmRemoteProvider: Boolean,
)

@Serializable private data class ChatSessionRequest(
...
@Serializable private data class ChatMessageRequest(
    val message: String,
...
@Serializable private data class DraftUpdateRequest(
    @SerialName("project_revision") val projectRevision: String,
...
@Serializable private data class DraftRevisionRequest(
    @SerialName("project_revision") val projectRevision: String,
    @SerialName("expected_revision") val expectedRevision: Long,
)

@Serializable private data class DraftCheckRequest(
    @SerialName("project_revision") val projectRevision: String,
    @SerialName("expected_revision") val expectedRevision: Long,
...
@Serializable private data class DraftApplyRequest(
...
@Serializable private data class DraftUndoRequest(
    @SerialName("project_id") val projectId: String,
    @SerialName("project_revision") val projectRevision: String,
...
class ApiClient(
    ...
    fun importProject(path: String, confirmRemoteProvider: Boolean = false): ProjectAnalysis =
        decode(send("POST", "/api/projects/import", requestBody(ProjectImportRequest(path, confirmRemoteProvider))))
    ...
    fun reindex(revision: String): ProjectIndex =
        decode(send("POST", "/api/projects/current/reindex", requestBody(ProjectRevisionRequest(revision))))
    ...
    fun analyze(path: String, revision: String, refresh: Boolean = false, confirmRemoteProvider: Boolean = false): FileAnalysis =
        decode(send("POST", "/api/projects/current/files/analysis", requestBody(FileAnalysisRequest(path, revision, refresh, confirmRemoteProvider))))
    ...
    fun updateFindingStatus(findingId: String, revision: String, status: String) {
        sendNoContent("PATCH", "/api/projects/current/findings/${encodePath(findingId)}", requestBody(FindingStatusRequest(revision, status)))
    }
    ...
    fun startGoScan(revision: String): GoScanReport =
        decode(send("POST", "/api/projects/current/scan", requestBody(ProjectRevisionRequest(revision))))
    ...
    fun startAnalyzeAll(revision: String, maxFiles: Int = 0, maxRetries: Int = 0, confirmRemoteProvider: Boolean = false): AnalyzeAllJob =
        decode(send("POST", "/api/projects/current/analysis-job", requestBody(AnalyzeAllStartRequest(revision, maxFiles, maxRetries, confirmRemoteProvider))))
    ...
    fun resumeAnalyzeAll(revision: String, confirmRemoteProvider: Boolean = false): AnalyzeAllJob =
        decode(send("POST", "/api/projects/current/analysis-job/resume", requestBody(AnalyzeAllResumeRequest(revision, confirmRemoteProvider))))
    ...
    fun openChatSession(projectId: String, revision: String, baseFileHash: String, openPath: String, mode: String, targetSymbol: String, taskSpec: BugTaskSpec? = null): ChatSession =
        decode(send("POST", "/api/projects/current/chat/sessions", requestBody(ChatSessionRequest(projectId, revision, baseFileHash, openPath, mode, targetSymbol, taskSpec))))

    fun sendChatMessage(sessionId: String, message: String, parentDraftId: String = "", confirmRemoteProvider: Boolean = false, repair: Boolean = false): ChatDraftProposal =
        decode(send("POST", "/api/projects/current/chat/sessions/${encodePath(sessionId)}/messages", requestBody(ChatMessageRequest(message, parentDraftId, confirmRemoteProvider, repair))))

    // Draft review and guarded write routes.
    fun updateDraft(draftId: String, projectRevision: String, expectedRevision: Long, declaration: String, imports: List<String>): DeclarationDraft =
        decode(send("PATCH", "/api/projects/current/drafts/${encodePath(draftId)}", requestBody(DraftUpdateRequest(projectRevision, expectedRevision, declaration, imports))))

    fun validateDraft(draftId: String, projectRevision: String, expectedRevision: Long): DeclarationDraft =
        decode(send("POST", "/api/projects/current/drafts/${encodePath(draftId)}/validate", requestBody(DraftRevisionRequest(projectRevision, expectedRevision))))

    fun checkDraft(draftId: String, projectRevision: String, expectedRevision: Long, expectedHash: String, runLint: Boolean = false, runTests: Boolean = false): DraftCheckReport =
        decode(send("POST", "/api/projects/current/drafts/${encodePath(draftId)}/checks", requestBody(DraftCheckRequest(projectRevision, expectedRevision, expectedHash, runLint, runTests))))

    fun applyDraft(draft: DeclarationDraft): ApplyResult =
        decode(send("POST", "/api/projects/current/apply", requestBody(DraftApplyRequest(draft.id, draft.revision, draft.hash, draft.projectId, draft.projectRevision, draft.baseFileHash, true))))
    ...
    fun undo(projectId: String, revision: String, postApplyHash: String): ApplyResult =
        decode(send("POST", "/api/projects/current/undo", requestBody(DraftUndoRequest(projectId, revision, postApplyHash, true))))
    ...
    private fun decodeApiError(body: String): ApiError? = try {
