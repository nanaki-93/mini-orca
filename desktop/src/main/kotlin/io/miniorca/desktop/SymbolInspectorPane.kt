    ...
    Column(modifier.verticalScroll(rememberScrollState()).padding(12.dp)) {
        ...
        if (inspector.analysisAction == InspectorAnalysisAction.None && inspector.remoteProviderConfirmationRequired) {
            ...
            FocusFlowPanel(Modifier.fillMaxWidth()) {
                ...
                RemoteProviderConfirmation(ModelScope.Bug, state.bugModel, state.remoteProviderConfirmed, actions.confirmRemoteProvider)
            }
        }
        if (inspector.analysisAction != InspectorAnalysisAction.None) {
            val onAnalysisAction: () -> Unit = when (inspector.analysisAction) {
                ...
                InspectorAnalysisAction.CancelAnalysis, InspectorAnalysisAction.None -> actions.cancel
            }
            Spacer(Modifier.height(10.dp))
            FocusFlowPanel(Modifier.fillMaxWidth()) {
                ...
                FocusFlowButton(
                    ...
                    enabled = !inspector.remoteProviderConfirmationRequired || state.remoteProviderConfirmed,
