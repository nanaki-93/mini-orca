...
/**
 * Resolves overlapping declarations predictably: shortest range first, then an exact atomic target,
 * then the order returned by the index.
 */
fun symbolAtLine(symbols: List<SymbolInfo>, line: Int): SymbolInfo? =
    if (line <= 0) null
        ...
        symbols
            .withIndex()
            .filter { (_, symbol) ->
              symbol.hasValidRange() && line in symbol.startLine..symbol.endLine
            }
            .minWithOrNull(
...
enum class SymbolInspectorMode {
  FileFallback,
  SelectedSymbol
}
  ...
  val symbolState =
      selectedSymbol?.let { symbol ->
        ...
        SymbolInspectorSymbolState(
            ...
            currentDraftBoundToAnotherTarget =
                currentEditIdentity?.hasDraft == true && !currentEditIdentity.matches(file, symbol),
        )
      }
  return SymbolInspectorUiState(
      mode =
          if (symbolState == null) SymbolInspectorMode.FileFallback
          else SymbolInspectorMode.SelectedSymbol,
      file = file,
...
fun symbolEditEligibility(
  ...
  val validation =
      validateChatTarget(selectedFile, symbols, selectedSymbol, ChatEditMode.ReplaceSymbol, "")
...
fun currentEditIdentity(
    ...
    currentDraftEditIdentity(draft, selectedFile, project)
...
private fun currentDraftEditIdentity(
  ...
  if (currentDraft.targetPath != file.path ||
      currentDraft.baseFileHash != file.contentHash ||
      currentDraft.projectId != activeProject.projectId ||
      currentDraft.projectRevision != activeProject.projectRevision ||
      currentDraft.targetSymbol.isBlank())
      return null
  return CurrentEditIdentity(
      mode, currentDraft.targetPath, currentDraft.targetSymbol, hasDraft = true)
}
...
private fun currentSessionEditIdentity(
  ...
  if (target.symbol.isBlank() || !chatSessionMatches(currentSession, selectedFile, project, target))
      return null
  return CurrentEditIdentity(
      mode, currentSession.openPath, currentSession.targetSymbol, hasDraft = false)
}

private fun String.toChatEditMode(): ChatEditMode? =
    ChatEditMode.entries.firstOrNull { it.wireValue == this }

private fun inspectorAnalysisStatus(
    analysis: FileAnalysis?,
    analysisInProgress: Boolean
): InspectorAnalysisStatus =
    when {
      analysisInProgress || analysis?.status.equals("running", ignoreCase = true) ->
          InspectorAnalysisStatus.Running
      analysis == null || analysis.status.equals("missing", ignoreCase = true) ->
          InspectorAnalysisStatus.Missing
...
private fun InspectorAnalysisStatus.action(): InspectorAnalysisAction =
    when (this) {
      InspectorAnalysisStatus.Missing,
      InspectorAnalysisStatus.Failed -> InspectorAnalysisAction.AnalyzeFile
...
private fun symbolConfidenceLabel(symbol: SymbolInfo, eligibility: SymbolEditEligibility): String =
    when {
      ...
      symbol.confidence.equals("exact", ignoreCase = true) ->
          "Exact indexed declaration · read-only"
...
/**
 * Derives contextual progress from existing selection, chat, draft, validation, check, and receipt
 * truth.
 */
fun editorProgressUiState(state: DesktopState): EditorProgressUiState {
  ...
  if (receipt != null) {
    return EditorProgressUiState(
        EditorProgress.Receipt, "Change applied. Guarded Undo is available.")
  }
  ...
  return if (validationCurrent) {
    EditorProgressUiState(
        EditorProgress.Review, "Validation is current for ${identity.targetSymbol}.", identity)
  } else {
