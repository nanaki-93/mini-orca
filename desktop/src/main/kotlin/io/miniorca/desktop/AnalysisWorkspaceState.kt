...
private fun AnalysisCoverage?.toPresentation() = AnalysisCoveragePresentation(
    total = this?.total ?: 0,
    fresh = this?.fresh ?: 0,
    stale = this?.stale ?: 0,
...
private fun AnalyzeAllFileJob.toFailurePresentation() = AnalysisFailurePresentation(
    path = path,
    attempts = attempts,
    error = error.trim().ifBlank { "Analysis failed" },
)

private fun AnalyzeAllJob?.statusLabel() = when (this?.status.normalizedAnalyzeAllStatus()) {
    null -> "Not started"
    "running" -> "Running"
    "pausing" -> "Pausing"
    "canceling" -> "Canceling"
...
private fun AnalyzeAllJob?.controls() = when (this?.status.normalizedAnalyzeAllStatus()) {
    ...
    "failed" -> "Retry"
    "stale", "completed", "canceled" -> "Start a new run"
    else -> "Start"
}
