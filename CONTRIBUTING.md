...
5.  **Write tests** for behavior changes and run the full local validation path: `make check` plus `make quality`. Use `./desktop/gradlew -p desktop test` for desktop-only feedback; the checked-in wrapper avoids requiring a globally installed Gradle.
...
- `make quality` downloads pinned Go analyzers into user-level Go and npm caches
  through `go run` and `npx`, then runs the checked-in Gradle wrapper's Kotlin
  format and Detekt gates. None of these tools are runtime dependencies or
  modify `go.mod`.
